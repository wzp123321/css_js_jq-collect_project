...
$(function(){
	...
	$(".nav>li").hover(function(){
		$(this).find(".sub_box").stop().slideDown(300);
	},function(){
		$(this).find(".sub_box").stop().slideUp(300);
	});
	//dl_icon 高度
	var dl_width = $(".dl_icon").width();
	$(".dl_icon").css("height",dl_width);
	$(window).resize(function(){
		var dl_width1 = $(".dl_icon").width();
		$(".dl_icon").css("height",dl_width1);
	});
	//招聘列表
	$(".join_ul li .join_title").click(function(){
		$(this).parent("li").toggleClass("on");
		$(this).siblings(".join_content").slideToggle(500);
	});
	//放大镜滚动
	...
	//相关产品，产品详情悬浮导航
	function fixed(){
		var _top = $(".pro_title_wrap span").offset().top;
		var wtop = $(window).scrollTop();
		if(_top < wtop){
			$(".fixed_title_wrap").show();
		}else{
			$(".fixed_title_wrap").hide();
		};
		var _top1 = $("#pro2").offset().top;
		if(wtop >= (_top1-30)){
			$(".fixed_title_wrap span:eq(1)").addClass("on").siblings("span").removeClass("on");
		}else{
			$(".fixed_title_wrap span:eq(0)").addClass("on").siblings("span").removeClass("on");
		};
