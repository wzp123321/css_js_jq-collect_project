...
$(function(){
	...
	$(".search_icon").click(function(){
		$(".search_box").addClass("search_on");
		$(".search_box").animate({width:"300px"},300);
		$(this).hide();
		$(".nav").animate({opacity:"0"},300);
	});
	$(".search_cha").click(function(){
		$(".search_box").animate({width:"28px"},300).removeClass('search_on');
		$(".search_box").delay(1000).removeClass("search_on");
		$(".search_icon").show();
		$(".nav").animate({opacity:"1"},300);
	})
	
	//资质荣誉
	$('#honor_scroll').owlCarousel({
		navigation: true,
		autoPlay: true,
		items: 5,
		navigationText: ["<",">"]
	});
	$("#honor_scroll .item img").click(function(){
		var _data = $(this).attr("data");
		$(".big_honor img").attr("src",_data);
	});
	
	
	
	//新闻伸缩功能
	$(".news_ul li span").click(function(){
		$(this).siblings(".news_ms").stop().slideToggle(300);
		$(this).toggleClass("span_on");
	});
	//产品
	$('#name_scroll').owlCarousel({
		navigation: true,
		autoPlay: true,
		items: 8,
		navigationText: ["<",">"]
